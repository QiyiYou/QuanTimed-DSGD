    ...
    for iteration = 1: niterations 
        for j= 1: nworkers % for workers 1,2,3,...,50
            ...
            gradnew(:,j) = nnCostFunctionDGD_four_layers(nn_params(:,j), input_layer_size, hidden_layer1_size,...
                ...
                num_labels, features((j-1) * floor(size(features,1)/nworkers)+1:j*floor(size(features,1)/nworkers),:), label((j-1)*floor(size(features,1)/nworkers)+1:j*floor(size(features,1)/nworkers)), lambda,T,batchsize);
        ...
        cost (iteration) = costf_four_layers (mean(nn_params,2), features, label,lambda,input_layer_size, hidden_layer1_size,...
