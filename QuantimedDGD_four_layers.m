...
for iteration = 1: niterations 
 for j= 1: nworkers % for workers 1,2,3,...,50
  gradnew(:,j) = nnCostFunction_four_layers(nn_params(:,j), input_layer_size, hidden_layer1_size,...
                  ...
                  num_labels, Xnewnew((j-1) * floor(size(Xnewnew,1)/nworkers)+1:j*floor(size(Xnewnew,1)/nworkers),:), labelstotal((j-1)*floor(size(Xnewnew,1)/nworkers)+1:j*floor(size(Xnewnew,1)/nworkers)), lambda,T,vmin,vmax);
