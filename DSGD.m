...
for iteration = 1: niterationsDGD % DSGD
 for j= 1: nworkers % for workers 1,2,3,...,50
   gradnew(:,j) = nnCostFunctionDGD(nn_params(:,j), input_layer_size, hidden_layer_size, ...
                  num_labels, Xnewnew((j-1)*floor(10000/nworkers)+1:j*floor(10000/nworkers),:), labelstotal((j-1)*floor(10000/nworkers)+1:j*floor(10000/nworkers)), lambda,T,mu,batchsize);
 ...
 costDGD (t2,iteration) = costf(mean(nn_params,2),Xnewnew,labelstotal,lambda,hidden_layer_size,input_layer_size,num_labels);
end
t2=t2+1 ;
end
